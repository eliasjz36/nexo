...
Normalización en cuatro capas, de la más segura a la más discrecional:
...
def main():
    ...
    entidades = {e for fila in con.execute(
        "SELECT sujeto, objeto FROM relaciones") for e in fila}
    ...
    con.executescript("""
        ...
        CREATE TABLE nodos (nombre TEXT PRIMARY KEY, menciones INTEGER,
            es_mesh INTEGER);
    """)
    for pmid, s, rel, o, frase in con.execute(
